house_price_data<- read_csv("duplicates_removed_UK_LA.csv")
...
gb_boundaries <- readOGR(dsn = "census/shapefiles", layer = "boundaries_gb")
#getinfo.shape("census/shapefiles/boundaries_gb.shp") 
#gb.map <- readShapeSpatial("census/shapefiles/boundaries_gb.shp")
#class(gb.map)
#head(gb.map@data$SHAPE_AREAS)
#gb_boundaries.map < - gb_boundaries.map[gb_boundaries.map$NAME == "Australia",]
#plot(australia.map)
...
# into the Console.
gb_boundaries@data <- left_join(gb_boundaries@data, house_price_data,  by=c("geo_code" =  "LA_Code"))
